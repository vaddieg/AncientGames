...
#import "fireTextView.h"

#define FIRE_WIDTH 300
#define FIRE_HEIGHT 90

@implementation fireTextView
...
- (id)initWithFrame:(NSRect)frame isPreview:(BOOL)isPreview
{
    ...
    if (self) {
		// todo get text and attrs
		NSDictionary* attrs = [NSDictionary dictionaryWithObjectsAndKeys:
			[NSColor whiteColor], NSForegroundColorAttributeName,
			[NSColor blackColor], NSBackgroundColorAttributeName,
			[NSFont systemFontOfSize:28], NSFontAttributeName, nil
		];
		NSAttributedString* as = [[NSAttributedString alloc] initWithString:@"VaD" attributes:attrs];
		NSSize size = [as size];
		NSImage* img = [[NSImage alloc] initWithSize:size];
		[img lockFocus];
		[as drawAtPoint:NSZeroPoint];
		[img unlockFocus];
		// Getting whitepoints coordinates
		int i,j;
		int m = size.width;
		int n = size.height;
